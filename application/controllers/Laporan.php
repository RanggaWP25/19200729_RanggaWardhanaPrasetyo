...
class Laporan extends CI_Controller
{
    ...
    public function laporan_buku_pdf()
    {
        ...
        $pdf->loadHtml($html);
        $pdf->render();
        $pdf->stream("laporan_data_buku.pdf", [
            'Attachment' => 0
        ]);
    }
    ...
    public function laporan_pinjam_pdf()
    {
        ...
        $pdf->loadHtml($html);
        $pdf->render();
        $pdf->stream("laporan_data_peminjaman.pdf", [
            'Attachment' => 0
        ]);
    }
    ...
    public function laporan_anggota()
    {
        ...
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['laporan'] = $this->db->query("SELECT nama, alamat, email, image, password FROM user")->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar');
        $this->load->view('templates/topbar', $data);
    ...
    public function laporan_anggota_pdf()
    {
        ...
        $pdf->loadHtml($html);
        $pdf->render();
        $pdf->stream("laporan_data_anggota.pdf", [
            'Attachment' => 0
        ]);
    }
    ...
    public function export_excel_anggota()
    {
        $data = array(
            ...
            'laporan' => $this->db->query("SELECT nama, alamat, email, image, password FROM user")->result_array()
        );
